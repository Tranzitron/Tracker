...
- `SettingsPage`'s remaining snackbar placeholders (profile/notifications/privacy) are still to be built (Milestone 7), as is the Feed activity-stream content. `ExerciseDetailPage` shows only the best-1RM chart; further performance-log detail is optional polish.
