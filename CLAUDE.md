...
- `ExercisesPage` (`lib/pages/exercises_page.dart`) is the master exercise library: browse by muscle group or movement pattern (segmented toggle), with `NewExercisePage` (`lib/pages/exercises/new_exercise_page.dart`) for custom exercises and `ExerciseDetailPage` (`lib/pages/exercises/exercise_detail_page.dart`, §1.4.1.1) for the profile — historical stats/graphs are Milestone 6.
...
- `HistoryPage` per-session detail + calendar views, and setting pages' snackbar placeholders, are still to be built (Milestones 5/7). `ExerciseDetailPage` shows the profile; its historical graphs land with analytics (Milestone 6).
