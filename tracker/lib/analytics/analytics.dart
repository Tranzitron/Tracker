...
class AnalyticsService {
  ...
  AnalyticsSnapshot snapshot({
    ...
    final key = _AnalyticsKey(_fingerprint(sessions),
        _mapFingerprint(multipliers), exerciseId, revision);
...
class _AnalyticsKey {
  const _AnalyticsKey(
      this.sessions, this.multipliers, this.exerciseId, this.revision);
...
int _fingerprint(List<WorkoutSession> sessions) {
  ...
  for (final s in sessions) {
    hash = Object.hash(hash, s.id, s.title, s.startTime.microsecondsSinceEpoch,
        s.endTime?.microsecondsSinceEpoch, s.gymId);
    for (final set in s.sets) {
      hash = Object.hash(
          hash, set.exerciseId, set.weight, set.reps, set.type.index);
    }
  }
  return hash;
}
...
List<ProgressionPoint> exerciseBest1rm(List<WorkoutSession> sessions,
    Map<int, double> multipliers, int? exerciseId) {
...
List<ProgressionPoint> exercisePeakWeight(List<WorkoutSession> sessions,
    Map<int, double> multipliers, int? exerciseId) {
  final points = <ProgressionPoint>[];
  for (final session in sessions) {
    final sets = _workingSets(session)
...
List<ProgressionPoint> volumeTrend(
    List<WorkoutSession> sessions, Map<int, double> multipliers) {
  final points = <ProgressionPoint>[];
  for (final session in sessions) {
    final sets = _workingSets(session);
    if (sets.isEmpty) continue;
    points.add(ProgressionPoint(
        ...
        value: sets.fold(
            ...
            (sum, s) =>
                sum +
                normalizedWeight(s.weight, multipliers, session.gymId) *
                    s.reps)));
  }
  points.sort((a, b) => a.date.compareTo(b.date));
  return points;
...
ExerciseSummary exerciseSummary(List<WorkoutSession> sessions,
    Map<int, double> multipliers, int? exerciseId) {
  var count = 0;
  var peak = 0.0;
  var best = 0.0;
...
double? estimateGymMultiplier(
    List<WorkoutSession> sessions, int? primaryGymId, int gymId) {
